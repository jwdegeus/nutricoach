...
type DietRuleSet = {
  ...
  prepTimeConstraints: { globalMax?: number; perMeal?: PrepTimePreference; batchCooking?: BatchCookingPreference };
...
ingredientConstraints: [
  ...
  ...(profile.allergies.length > 0 ? [{ type: "forbidden", items: profile.allergies, constraintType: "hard" }] : []),
]
```
...
console.log(`[RecipeAdaptation] Found ${constraints?.length || 0} guard rail constraints`);
console.log(`[RecipeAdaptation] Guard rails loaded: ${forbidden.length} forbidden terms`);

// Validation
console.log(`[DietValidator] Checking text: "${text}" against ${ruleset.forbidden.length} forbidden rules`);
...
**Code**:
```typescript
const constraintIssues = await validateHardConstraints({ plan, rules, request });
