-- Migration: Add recipe_sources upsert to finalize_recipe_import
-- Created: 2026-01-31
-- Description: Automatically create or update recipe_sources when finalizing recipe imports

-- Drop and recreate the function to include recipe_sources upsert
...
BEGIN
  ...
  -- IMPORTANT: Prefer savedImageUrl (downloaded and saved locally) over external imageUrl
  IF v_job.source_image_meta IS NOT NULL THEN
    ...
    -- Extract image URL - CRITICAL: Check savedImageUrl FIRST
  ...
  -- Step 0: Upsert recipe_sources if domain is available
  -- This ensures imported sources are tracked in the recipe_sources table
  -- If source doesn't exist: create it with usage_count = 1
  -- If source exists: increment usage_count by 1
  IF v_source_domain IS NOT NULL AND TRIM(v_source_domain) != '' THEN
    ...
    VALUES (
      TRIM(v_source_domain),
      false, -- User-created source from import
      v_user_id,
      1 -- First usage
    )
    ON CONFLICT (name) DO UPDATE
    SET
      usage_count = recipe_sources.usage_count + 1,
      updated_at = NOW();
  END IF;
