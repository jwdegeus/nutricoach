-- Migration: RPC Function for Atomic Recipe Import Finalization
-- Created: 2026-01-30
-- Description: Postgres RPC function voor atomische finalize van recipe imports

-- ============================================================================
-- Function: finalize_recipe_import
-- ============================================================================
-- Atomisch finaliseren van een recipe import job:
-- 1. Lock job row (FOR UPDATE) om race conditions te voorkomen
-- 2. Check ownership (user_id = auth.uid())
-- 3. Check status = 'ready_for_review'
-- 4. Idempotency: als al finalized met recipe_id -> return recipe_id
-- 5. Validate extracted_recipe_json is not null
-- 6. Insert custom_meals
-- 7. Bulk insert recipe_ingredients
-- 8. Update recipe_imports (status, recipe_id, finalized_at)
-- 9. Return recipe_id
--
-- Alles gebeurt in één database transaction.
...
BEGIN
  ...
  -- Extract servings (nullable)
  v_servings := CASE 
    WHEN (v_extracted_recipe->>'servings')::INTEGER > 0 
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    RAISE EXCEPTION 'DB_ERROR: Failed to finalize recipe import: %', SQLERRM;
END;
$$;

-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION public.finalize_recipe_import(UUID, TEXT) TO authenticated;
