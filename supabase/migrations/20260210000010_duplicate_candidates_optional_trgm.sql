-- Make trigram matching optional to avoid statement timeout on large NEVO/FNDDS sets.
-- Default: exact + contains only (fast). Set p_include_trgm = true for fuzzy (can be slow).
...
COMMENT ON FUNCTION public.find_ingredient_duplicate_candidates(text, int, float, boolean, boolean) IS
  'Returns NEVO ↔ FNDDS duplicate candidates. exact+contains always; trgm only when p_include_trgm=true (can be slow).';

-- Drop old 4-arg version so only 5-arg exists (defaults make it backward compatible)
DROP FUNCTION IF EXISTS public.find_ingredient_duplicate_candidates(text, int, float, boolean);

GRANT EXECUTE ON FUNCTION public.find_ingredient_duplicate_candidates(text, int, float, boolean, boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.find_ingredient_duplicate_candidates(text, int, float, boolean, boolean) TO anon;
