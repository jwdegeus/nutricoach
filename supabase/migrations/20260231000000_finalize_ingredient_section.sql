-- Migration: Finalize - include ingredient section in meal_data
-- Description: When extracted_recipe_json.ingredients have "section" (from URL import),
--              persist it in meal_data.ingredients so the recipe page shows section headings.
...
BEGIN
  ...
  -- Include optional section from extracted ingredients (for URL imports with grouped ingredients)
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'section', CASE WHEN section IS NOT NULL AND TRIM(section) != '' THEN TRIM(section) ELSE NULL END
  ))
  INTO v_ingredients
