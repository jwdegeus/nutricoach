 ...
 */
function getRunType(action: PlanEdit["action"]): "generate" | "regenerate" | "enrich" {
  switch (action) {
    case "REGENERATE_DAY":
      return "regenerate";
    case "REPLACE_MEAL":
  ...
  try {
    ...
    if (runError || !runData) {
      return {
        ...
        error: {
          code: "DB_ERROR",
          message: "Fout bij starten edit: " + (runError?.message || "Unknown error"),
        },
      };
    }
    ...
    applyPlanEdit({
      ...
      .catch(async (error) => {
        ...
        const duration = Date.now() - startTime;
        const errorCode = error instanceof AppError ? error.code : "AGENT_ERROR";
        const errorMessage = error instanceof AppError ? error.safeMessage : (error instanceof Error ? error.message : "Unknown error");
 ...
 */
export async function checkPlanEditStatusAction(
  runId: string
): Promise<ActionResult<{ status: "running" | "success" | "error"; errorMessage?: string }>> {
  try {
    const supabase = await createClient();
    const {
      data: { user },
...
export async function getActivePlanEditsAction(
  planId: string
): Promise<ActionResult<Array<{ runId: string; runType: string; createdAt: string }>>> {
  try {
    const supabase = await createClient();
    const {
      data: { user },
