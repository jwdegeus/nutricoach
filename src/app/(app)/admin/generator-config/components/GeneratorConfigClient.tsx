...
export function GeneratorConfigClient({ initialData, loadError }: Props) {
  ...
  const [previewTherapeuticTargets, setPreviewTherapeuticTargets] =
    useState<TherapeuticTargetsSnapshot | null>(null);
  ...
  const handlePreviewGenerate = () => {
    ...
    setPreviewTherapeuticTargets(null);
    ...
    startTransition(async () => {
      ...
      setPreviewTherapeuticTargets(result.therapeuticTargets ?? null);
  ...
  function getPresetPayload(
    ...
    return { ...DEFAULT_SETTINGS, ...preset } as PresetSettingsPayload;
  }
    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
              Beheer templates, pools en instellingen voor de
              weekmenu-generator.
            </p>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1 space-y-2">
              ...
              <Text className="text-sm text-muted-foreground">
                Weekmenu&apos;s worden primair gevuld uit je receptenbank.
                Onderstaande instellingen zijn legacy en gelden alleen als
        ...
        <Disclosure as="div" className="space-y-2">
          {({ open }) => (
            <>
              <DisclosureButton className="flex w-full items-center justify-between rounded-lg py-3 text-left text-sm font-medium text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-zinc-900">
                <span>Geavanceerd (legacy)</span>
                <ChevronRightIcon
                  className={`size-5 flex-shrink-0 transition-transform ${open ? 'rotate-90' : ''}`}
                  aria-hidden
                />
              </DisclosureButton>
                ...
                {activeTab === 'naming' && (
                  <div className="space-y-4">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                      <div className="flex flex-wrap items-center gap-4">
                        <div className="flex items-center gap-2">
                          ...
                          <Listbox
                            value={namingDietKey}
                            onChange={(v) =>
                              setNamingDietKey((v as string) ?? 'default')
                            }
                            placeholder="default"
                            aria-label="Filter op dieet"
                            className="min-w-[140px]"
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Listbox
                            ...
                            onChange={(v) =>
                              setNamingTemplateKey((v as string) ?? '')
                            }
                            placeholder="Alle"
                      ...
                      <Button onClick={openCreateNamePattern}>
                        Nieuw patroon
                      </Button>
                    </div>
                  ...
                  <div className="max-w-xl space-y-6">
                    <Field>
                      ...
                      <Listbox
                        ...
                        onChange={(v) =>
                          setSettingsDietKey((v as string) ?? 'default')
                        }
                        placeholder="default"
                    ...
                    <Fieldset>
                      ...
                      <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                        Drempels (g) en scores (0–20) voor groente-doel; low ≤
                        mid ≤ high.
                      </p>
                      <FieldGroup className="mt-3">
                        <Field>
                          <Label>Drempel laag (g)</Label>
                          <Input
                            type="number"
                            min={1}
                            max={2000}
                            value={settingsForm.veg_threshold_low_g}
                            onChange={(e) => {
                              setSettingsForm((s) => ({
                                ...s,
                                veg_threshold_low_g:
                        ...
                        <Field>
                          <Label>Drempel mid (g)</Label>
                          <Input
                            type="number"
                            min={1}
                            max={2000}
                            value={settingsForm.veg_threshold_mid_g}
                            onChange={(e) => {
                              setSettingsForm((s) => ({
                                ...s,
                                veg_threshold_mid_g:
                        ...
                        <Field>
                          <Label>Drempel hoog (g)</Label>
                          <Input
                            type="number"
                            min={1}
                            max={2000}
                            value={settingsForm.veg_threshold_high_g}
                            onChange={(e) => {
                              setSettingsForm((s) => ({
                                ...s,
                                veg_threshold_high_g:
                                  parseInt(e.target.value, 10) || 250,
                              }));
                              setSettingsDirty(true);
                            }}
                          />
                        ...
                        <Field>
                          <Label>Score laag</Label>
                          <Input
                            type="number"
                            min={0}
                            max={20}
                            value={settingsForm.veg_score_low}
                            onChange={(e) => {
                              setSettingsForm((s) => ({
                                ...s,
                                veg_score_low:
                        ...
                        <Field>
                          <Label>Score mid</Label>
                          <Input
                            type="number"
                            min={0}
                            max={20}
                            value={settingsForm.veg_score_mid}
                            onChange={(e) => {
                              setSettingsForm((s) => ({
                                ...s,
                                veg_score_mid:
                        ...
                        <Field>
                          <Label>Score hoog</Label>
                          <Input
                            type="number"
                            min={0}
                            max={20}
                            value={settingsForm.veg_score_high}
                            onChange={(e) => {
                              setSettingsForm((s) => ({
                                ...s,
                                veg_score_high:
              ...
              <div className="space-y-3">
                {suggestMeta && suggestMeta.guardrailsTermsCount > 0 && (
                  <p className="text-sm text-zinc-500 dark:text-zinc-400">
                    ...
                    terms, verwijderd:{' '}
                    {suggestMeta.removedByGuardrailsTerms ?? 0}
                  </p>
                )}
                <div className="flex items-center gap-2">
                  <Button
          ...
          <DialogBody>
            ...
            {!previewResult ? (
              ...
              <div className="space-y-4">
                  ...
                  'generator' in previewResult.metadata && (
                    <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-3 text-sm dark:border-zinc-700 dark:bg-zinc-900/50">
                      ...
                      {(() => {
                        ...
                        const termsCount =
                          gen?.guardrailsExcludeTermsCount ?? 0;
                        ...
                        return showGuardrails ? (
                          <ul className="mb-2 space-y-0.5 text-xs text-zinc-500 dark:text-zinc-400">
                            ...
                            {removedByGuardrails > 0 && (
                              <li>
                                Verwijderd door guardrails:{' '}
                                {removedByGuardrails} items
                              </li>
                            )}
                          </ul>
                        ) : null;
                      ...
                      {(() => {
                        ...
                        ).therapeuticCoverage?.deficits;
                        const alertCount = Array.isArray(defs?.alerts)
                          ? defs.alerts.length
                          : 0;
                        const suggestionCount = Array.isArray(defs?.suggestions)
                          ? defs.suggestions.length
                          : 0;
                        const showTherapeutic =
                          alertCount > 0 || suggestionCount > 0;
                        return showTherapeutic ? (
                          <ul className="mb-2 space-y-0.5 text-xs text-zinc-500 dark:text-zinc-400">
                            {alertCount > 0 && (
                              <li>Therapeutische deficits: {alertCount}</li>
                            )}
                            {suggestionCount > 0 && (
                              <li>
                                Therapeutische suggesties: {suggestionCount}
                              </li>
                            )}
                          </ul>
                        ) : null;
                ...
                {previewTherapeuticTargets &&
                  (() => {
                    const t = previewTherapeuticTargets;
                    const protocol = t.protocol;
                    const daily = t.daily;
                    const supplements = t.supplements ?? [];
                    const targetEntries: Array<{ key: string; label: string }> =
                      [];
                    if (daily?.macros && typeof daily.macros === 'object') {
                      for (const [k, v] of Object.entries(daily.macros)) {
                        if (
                          v &&
                          typeof v === 'object' &&
                          'kind' in v &&
                          ...
                          const val = Number((v as { value: number }).value);
                          const label =
                            (v as { kind: string }).kind === 'adh_percent'
                              ? `${k}: ${val}% ADH`
                              : `${k}: ${val}${(v as { unit?: string }).unit ?? ''}`;
                          targetEntries.push({ key: k, label });
                        }
                      }
                    }
                    if (daily?.micros && typeof daily.micros === 'object') {
                      for (const [k, v] of Object.entries(daily.micros)) {
                        if (
                          v &&
                          typeof v === 'object' &&
                          'kind' in v &&
                    ...
                    const targetsSlice = targetEntries.slice(0, 6);
                    const foodGroups = daily?.foodGroups;
                    const vegetablesG =
                      foodGroups &&
                      typeof foodGroups === 'object' &&
                      'vegetablesG' in foodGroups
                        ? Number(
                            (foodGroups as { vegetablesG?: number })
                              .vegetablesG,
                          )
                        : undefined;
                    const fruitG =
                      foodGroups &&
                      typeof foodGroups === 'object' &&
                      'fruitG' in foodGroups
                        ? Number((foodGroups as { fruitG?: number }).fruitG)
                        : undefined;
                    const hasFoodGroups =
                      typeof vegetablesG === 'number' ||
                      typeof fruitG === 'number';
                    const computedAt =
                      typeof t.computedAt === 'string'
                        ? new Date(t.computedAt).toLocaleString('nl-NL', {
                            dateStyle: 'short',
                            timeStyle: 'short',
                          })
                        : null;
                      ...
                      <div className="rounded-lg border border-border bg-muted/30 p-3 text-sm">
                        ...
                        {protocol && (
                          <p className="text-foreground">
                            {protocol.labelNl ?? protocol.protocolKey}
                            {protocol.version != null && protocol.version !== ''
                              ? ` (v${protocol.version})`
                              : ''}
                          </p>
                        )}
                        ...
                        {targetsSlice.length > 0 && (
                          <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
                            {targetsSlice.map(({ key, label }) => (
                              <li key={key}>{label}</li>
                            ))}
                          </ul>
                        )}
                        {supplements.length > 0 && (
                          <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
                            {supplements.slice(0, 5).map((s) => (
                              <li key={s.key}>
                                {s.labelNl}
                                {s.dosageText != null && s.dosageText !== ''
                                  ? ` — ${s.dosageText}`
                                  : ''}
              ...
              <div className="space-y-4">
                <Field>
                  ...
                  <Listbox
                    value={exportDietKey}
                    onChange={(v) =>
                      setExportDietKey((v as string) ?? 'default')
                    }
        ...
        >
          ...
          <DialogBody>
            <p className="mb-2 text-sm text-zinc-500 dark:text-zinc-400">
              Plak hieronder een eerder geëxporteerde JSON. Importeer
              overschrijft bestaande waarden via upsert.
            </p>
            {importError && (
              <div
                role="alert"
