...
export const geminiIngredientSchema = z.object({
  original_line: z.string().describe("Original ingredient line as it appears in the image"),
  quantity: z.number().nullable().describe("Numeric quantity if extractable, null otherwise"),
  unit: z.string().nullable().describe("Unit of measurement if extractable (e.g., 'g', 'ml', 'cups'), null otherwise"),
  name: z.string().describe("Ingredient name (normalized)"),
  note: z.string().nullable().describe("Optional note or additional information about the ingredient"),
});
...
export const geminiTimesSchema = z.object({
  prep_minutes: z.number().int().min(0).nullable().describe("Preparation time in minutes"),
  cook_minutes: z.number().int().min(0).nullable().describe("Cooking time in minutes"),
  total_minutes: z.number().int().min(0).nullable().describe("Total time in minutes (prep + cook)"),
});
...
export const geminiConfidenceSchema = z.object({
  overall: z.number().min(0).max(100).nullable().describe("Overall confidence score (0-100)"),
  fields: z.record(z.string(), z.number().min(0).max(100)).optional().describe("Field-specific confidence scores"),
});
...
export const geminiExtractedRecipeSchema = z.object({
  ...
  instructions: z.array(geminiInstructionStepSchema).min(1).describe("List of cooking instructions (at least one required)"),
  confidence: geminiConfidenceSchema.optional().describe("Confidence scores"),
  warnings: z.array(z.string()).optional().describe("Optional warnings about extraction quality or missing information"),
});
