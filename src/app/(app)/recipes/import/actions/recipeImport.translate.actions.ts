  ...
  try {
    ...
    if (!job.extracted_recipe_json) {
      return {
        ...
        error: {
          code: "VALIDATION_ERROR",
          message: "Geen geëxtraheerd recept gevonden. Extracteer eerst het recept.",
        },
      };
    }

    // Get user's language preference for translation target
    const { ProfileService } = await import("@/src/lib/profile/profile.service");
    ...
    const originalExtracted = (job.original_recipe_json || job.extracted_recipe_json) as GeminiExtractedRecipe;
    ...
    if (sourceLocale === targetLocale) {
      console.log(`[translateRecipeImportAction] Recipe already in target language ${targetLocale}, skipping translation`);
    ...
    if (currentExtracted && currentExtracted.translated_to === targetLocale) {
      console.log(`[translateRecipeImportAction] Recipe already translated to ${targetLocale}, skipping`);
      return { ok: true, data: undefined };
    }
    
    console.log(`[translateRecipeImportAction] Starting translation. Source: ${sourceLocale}, Target: ${targetLocale}`);
    console.log(`[translateRecipeImportAction] Original title: "${extracted.title}"`);
    console.log(`[translateRecipeImportAction] Has original_recipe_json: ${!!job.original_recipe_json}`);
    console.log(`[translateRecipeImportAction] Current extracted_recipe_json.translated_to: ${currentExtracted?.translated_to || 'null'}`);

    console.log(`[translateRecipeImportAction] Translating recipe from ${sourceLocale} to ${targetLocale}`);
...
Translated title:`;
    ...
    if (extracted.ingredients && extracted.ingredients.length > 0) {
      ...
      const ingredientsList = extracted.ingredients.map((ing, idx) => {
        let text = ing.name || "";
...
Translated ingredients:`;
      ...
      try {
        ...
        const translatedIngredients = await Promise.all(
          extracted.ingredients.map(async (ing, idx) => {
            ...
            if (translatedLine) {
              ...
              const qtyUnitMatch = mainPart.match(/^([\d.,]+)\s*([a-zA-Z]+)\s+(.+)$/);
              if (qtyUnitMatch) {
            ...
            if (targetLocale === "nl" && translatedIng.unit && translatedIng.quantity) {
              const converted = convertUnitToDutch(translatedIng.unit, translatedIng.quantity);
        ...
        console.log(`[translateRecipeImportAction] Translated ${translatedIngredients.length} ingredients in batch`);
      } catch (error) {
        console.error(`[translateRecipeImportAction] Failed to translate ingredients in batch:`, error);
        // Fallback: translate individually
        console.log(`[translateRecipeImportAction] Falling back to individual translation...`);
        for (let i = 0; i < extracted.ingredients.length; i++) {
          ...
          if (ing.note) {
            const notePrompt = targetLocale === "nl"
            ...
            try {
              ...
              translatedIng.note = noteResponse.trim().replace(/^["']|["']$/g, '') || ing.note;
            } catch {
    ...
    if (extracted.instructions && extracted.instructions.length > 0) {
      ...
      let instructionsText = extracted.instructions.map((inst, idx) => {
        ...
        if (targetLocale === "nl" && sourceLocale === "en") {
          return text
            ...
            .replace(/(\d+)\s*-\s*(\d+)\s*°?\s*F/gi, (match, temp1, temp2) => {
...
Translated instructions:`;
      ...
      try {
        ...
        console.error(`[translateRecipeImportAction] Failed to translate instructions in batch:`, error);
        // Fallback: translate individually
        console.log(`[translateRecipeImportAction] Falling back to individual instruction translation...`);
        for (let i = 0; i < extracted.instructions.length; i++) {
          ...
          if (inst.text) {
            ...
            const textPrompt = targetLocale === "nl"
            ...
            try {
              ...
              translatedInst.text = textResponse.trim().replace(/^["']|["']$/g, '') || inst.text;
            } catch {
    ...
    if (updateError) {
      console.error("[translateRecipeImportAction] Error updating job:", updateError);
      return {
        ok: false,
        error: {
          code: "DB_ERROR",
    ...
    if (verifyJob) {
      const verifyTranslated = verifyJob.extracted_recipe_json as GeminiExtractedRecipe;
      console.log(`[translateRecipeImportAction] Verification: translated_to in DB = "${verifyTranslated?.translated_to || 'NOT SET'}"`);
    }

    console.log(`[translateRecipeImportAction] Translation completed successfully`);
