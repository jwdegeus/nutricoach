...
export function ImportStatusPanel({ state }: ImportStatusPanelProps) {
  ...
  const stateConfig: Record<ImportState, { labelKey: string; color: "green" | "red" | "yellow" | "zinc" | "blue"; icon: typeof CheckCircleIcon; descriptionKey: string }> = {
    idle: {
      labelKey: "statusIdle",
      color: "zinc",
      icon: ClockIcon,
  ...
  return (
    <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 p-4 bg-zinc-50 dark:bg-zinc-900/50">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className={`${state === "uploading" || state === "processing" ? "animate-spin" : ""}`}>
