...
export async function processRecipeImageWithGemini(args: {
  ...
  try {
    ...
    console.log(`[GeminiRecipeImport] Gemini API call completed in ${duration}ms, response length: ${rawResponse.length} chars`);
    ...
    if (error instanceof Error) {
      throw new Error(
        `Gemini recipe extraction failed: ${error.message}`
      );
    }
