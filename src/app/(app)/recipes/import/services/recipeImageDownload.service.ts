...
export async function downloadAndSaveRecipeImage(
  ...
  try {
    console.log(`[downloadAndSaveRecipeImage] Downloading image from: ${imageUrl}`);
    ...
    if (imageUrl.startsWith('/')) {
      ...
      console.warn(`[downloadAndSaveRecipeImage] Skipping relative URL (needs base URL): ${imageUrl}`);
      return null;
    }
    ...
    console.log(`[downloadAndSaveRecipeImage] Using absolute URL: ${absoluteUrl}`);
    ...
    if (!response.ok) {
      console.error(`[downloadAndSaveRecipeImage] Failed to fetch image: ${response.status} ${response.statusText}`);
      return null;
    }
    ...
    if (!contentType.startsWith('image/')) {
      console.error(`[downloadAndSaveRecipeImage] URL does not point to an image: ${contentType}`);
      return null;
    }
    ...
    console.log(`[downloadAndSaveRecipeImage] Image saved successfully: ${uploadResult.url}`);
    return uploadResult;
  } catch (error) {
    console.error(`[downloadAndSaveRecipeImage] Error downloading/saving image:`, error);
    return null;
  }
}
