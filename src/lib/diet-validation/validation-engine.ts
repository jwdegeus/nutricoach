...
function validateExcludeRule(
  ...
  if (rule.excludedCategories) {
    for (const category of rule.excludedCategories) {
      if (ingredient.category === category || ingredientMatchesCategory(ingredient.name, category)) {
...
function validateMealStructure(
  ...
  if (rule.vegetableCupsRequirement) {
    ...
    if (total < req.totalCups) {
      ...
      if (leafyCount < req.leafyCups) {
        incompletes.push(`ONVOLLEDIG: Alleen ${leafyCount} kopjes bladgroenten (${req.leafyCups} nodig)`);
      }
      if (sulfurCount < req.sulfurCups) {
        incompletes.push(`ONVOLLEDIG: Alleen ${sulfurCount} kopjes zwavelgroenten (${req.sulfurCups} nodig)`);
      }
      if (coloredCount < req.coloredCups) {
        incompletes.push(`ONVOLLEDIG: Alleen ${coloredCount} kopjes gekleurde groenten (${req.coloredCups} nodig)`);
      }
    }
  }
  ...
  if (rule.freshnessRequirement) {
    ...
    recipe.ingredients.forEach(ing => {
      // Check leftover age
      if (ing.freshness === "leftover" && ing.ageHours && ing.ageHours > req.maxLeftoverHours) {
        violations.push(
      ...
      if (ingredientMatchesCategory(ing.name, "meat") || ingredientMatchesCategory(ing.name, "poultry")) {
...
export function validateRecipeAgainstDiet(
  ...
  sortedRules.forEach(rule => {
    if (rule.ruleType === "require_ingredient") {
      ...
      const ruleIncompletes = validateRequireRule(recipe.ingredients, ruleValue);
