...
import type { EnrichedMeal, CookPlanDay, MealPlanEnrichmentResponse } from "@/src/lib/agents/meal-planner/mealPlannerEnrichment.types";
...
function detectLanguage(text: string): 'nl' | 'en' {
  ...
  const dutchWords = [
    'met', 'voor', 'van', 'een', 'de', 'het', 'en', 'op', 'is', 'zijn', 'in', 'te', 'dat',
    'koken', 'bakken', 'gebakken', 'gegrild', 'gestoomd', 'salade', 'snijden', 'mengen',
    'soep', 'pasta', 'rijst', 'kip', 'vlees', 'vis', 'groente', 'fruit', 'minuten',
    'voeg', 'toe', 'verwarm', 'serveer', 'snijd', 'mix', 'kook', 'bak', 'grill',
    'bereid', 'bereiding', 'kooktijd', 'voorbereiding', 'porties', 'keukentips'
  ];
  
  // Common English words and patterns (especially for cooking instructions)
  const englishWords = [
    'with', 'for', 'from', 'a', 'the', 'and', 'on', 'is', 'are', 'in', 'to', 'that',
    'cooked', 'baked', 'fried', 'grilled', 'steamed', 'salad', 'cut', 'mix', 'combine',
    'soup', 'pasta', 'rice', 'chicken', 'meat', 'fish', 'vegetable', 'fruit', 'minutes',
    'add', 'heat', 'serve', 'slice', 'blend', 'cook', 'bake', 'grill', 'prepare',
    'preparation', 'cooking', 'time', 'servings', 'kitchen', 'tips', 'until', 'until',
    'vigorously', 'smooth', 'frothy', 'immediately', 'pasteurized', 'consuming', 'raw'
  ];
  ...
  if (lowerText.includes('combine the') || lowerText.includes('blend or shake') || 
      lowerText.includes('serve immediately') || lowerText.includes('ensure the') ||
      lowerText.includes('if consuming') || lowerText.includes('use one pan')) {
    englishCount += 3; // Strong English indicators
  }
  
  // Check for common Dutch cooking instruction patterns
  if (lowerText.includes('voeg') || lowerText.includes('verwarm') || 
      lowerText.includes('serveer') || lowerText.includes('snijd') ||
      lowerText.includes('meng') || lowerText.includes('kook')) {
...
async function translateMealName(
  ...
  const prompt = targetLanguage === 'nl'
...
Translated name:`;
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes("quota")) {
      console.warn(`Translation skipped due to quota limit for meal: "${mealName}"`);
...
export async function translateMeals(
  ...
  for (let i = 0; i < mealsToTranslate.length; i += batchSize) {
    ...
    const batchPromises = batch.map(({ meal }) => translateMeal(meal, targetLanguage));
...
Translated text:`;
  ...
  try {
    ...
    // Handle quota errors gracefully - don't log as error, just return original
    if (error instanceof Error && error.message.includes("quota")) {
      console.warn(`Translation skipped due to quota limit for text: "${text.substring(0, 50)}..."`);
...
export async function translateEnrichedMeal(
  ...
  const translatedTitle = titleLanguage === targetLanguage 
    ? enrichedMeal.title 
  ...
  for (let i = 0; i < enrichedMeal.instructions.length; i++) {
    ...
    if (instructionLanguage === targetLanguage) {
      ...
      instructionsToTranslate.push({ text: enrichedMeal.instructions[i], index: i });
    }
  }
...
export async function translateEnrichment(
  ...
  const translatedMeals = await Promise.allSettled(
    enrichment.meals.map(meal => 
      translateEnrichedMeal(meal, targetLanguage)
    )
  ).then(results => 
  ...
  const translatedCookPlans = await Promise.allSettled(
    ...
    results.map((result, index) => 
      result.status === 'fulfilled' ? result.value : enrichment.cookPlanDays[index]
    )
  );
