...
export class MealScoringService {
   ...
   */
  calculateVarietyScore(usageCount: number, lastUsedAt: string | null): number {
  ...
  calculateCombinedScore(
    ...
    const normalizedRating = userRating ? ((userRating - 1) / 4) * 100 : 50; // Default to 50 if no rating
  ...
  async updateMealScores(userId: string, mealId: string): Promise<void> {
    ...
    if (updateError) {
      throw new Error(`Failed to update meal scores: ${updateError.message}`);
    }
  }
  ...
  async updateAllUserMealScores(userId: string): Promise<void> {
    ...
    const { data: meals, error: fetchError } = await supabase
      .from('meal_history')
      .select(
        'meal_id, user_rating, nutrition_score, usage_count, last_used_at',
      )
      .eq('user_id', userId);

    if (fetchError) {
      throw new Error(`Failed to load meal history: ${fetchError.message}`);
    }
