...
export async function getTermsForCategoryCodes(
  ...
  const { data: categories, error } = await supabase
    ...
    .select(
      "code, ingredient_category_items(term, synonyms, is_active)"
    )
  ...
  for (const cat of categories as Array<{ code: string; ingredient_category_items?: Array<{ term?: string; synonyms?: string[]; is_active?: boolean }> }>) {
 ...
 */
function ingredientMatchesTerms(text: string | undefined, terms: string[]): boolean {
 ...
 */
export function filterMealsByIngredientTerms<T extends { mealData?: { ingredientRefs?: Array<{ displayName?: string }>; ingredients?: Array<{ name?: string }> } }>(
  meals: T[],
  terms: string[]
): T[] {
  ...
  return meals.filter((m) => {
    ...
    const fromRefs = data.ingredientRefs?.some((r) => ingredientMatchesTerms(r.displayName, terms));
    if (fromRefs) return true;
    const fromLegacy = data.ingredients?.some((i) => ingredientMatchesTerms(i.name, terms));
    return !!fromLegacy;
  });
}
