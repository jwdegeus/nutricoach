...
export class ProfileService {
  ...
  async loadDietProfileForUser(userId: string): Promise<DietProfile> {
    ...
    const varietyLevel = mapDaysToVarietyLevel(
      preferences.variety_window_days
    );
    ...
    const profile: DietProfile = {
      ...
      strictness,
      ...((
        Array.isArray(preferences.breakfast_preference) && preferences.breakfast_preference.length > 0
      ) || (
        Array.isArray(preferences.lunch_preference) && preferences.lunch_preference.length > 0
      ) || (
        Array.isArray(preferences.dinner_preference) && preferences.dinner_preference.length > 0
      ) || (
        typeof preferences.breakfast_preference === 'string' && preferences.breakfast_preference.trim()
      ) || (
        typeof preferences.lunch_preference === 'string' && preferences.lunch_preference.trim()
      ) || (
        typeof preferences.dinner_preference === 'string' && preferences.dinner_preference.trim()
      )
        ? {
            mealPreferences: {
              breakfast: Array.isArray(preferences.breakfast_preference) && preferences.breakfast_preference.length > 0 
                ? preferences.breakfast_preference 
                : typeof preferences.breakfast_preference === 'string' && preferences.breakfast_preference.trim()
                  ? [preferences.breakfast_preference.trim()]
                  : undefined,
              lunch: Array.isArray(preferences.lunch_preference) && preferences.lunch_preference.length > 0 
                ? preferences.lunch_preference 
                : typeof preferences.lunch_preference === 'string' && preferences.lunch_preference.trim()
                  ? [preferences.lunch_preference.trim()]
                  : undefined,
              dinner: Array.isArray(preferences.dinner_preference) && preferences.dinner_preference.length > 0 
