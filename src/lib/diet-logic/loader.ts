...
function mapDbToConstraint(row: DbConstraint): DietLogicConstraint | null {
  const logic = (row.diet_logic ??
    (row.constraint_type === 'required' ? 'force' : 'drop')) as DietLogic;
...
export async function loadDietLogicRuleset(
  ...
  for (const row of (rows ?? []) as unknown as DbConstraint[]) {
  ...
  if (options?.isInflamed) {
    const nightshadeConstraints = await loadNightshadeDropConstraints(
      supabase,
      dietTypeId,
    );
  ...
  const byPriority = (a: DietLogicConstraint, b: DietLogicConstraint) =>
...
async function loadNightshadeDropConstraints(
  supabase: Awaited<
    ReturnType<(typeof import('@/src/lib/supabase/server'))['createClient']>
  >,
  dietTypeId: string,
  ...
  for (const cat of cats ?? []) {
    ...
    out.push({
      ...
      categoryNameNl:
        ((cat as { name_nl: string }).name_nl ?? 'Nachtschades') +
        ' (ontstekingsgevoelig)',
      terms: Array.from(terms),
      minPerDay: null,
      minPerWeek: null,
