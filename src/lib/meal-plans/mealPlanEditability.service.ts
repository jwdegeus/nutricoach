...
export class MealPlanEditabilityService {
  ...
  async checkMealEditability(
    ...
    // Load meal plan
    const plan = await this.mealPlansService.loadPlanForUser(userId, mealPlanId);
  ...
  private async getLock(
    ...
    if (error) {
      ...
      if (error.message.includes("Could not find the table") || 
        ...
        console.warn("meal_plan_locks table not found. Please run migration: npm run db:push");
        return null;
      }
  ...
  private async createLock(args: {
    ...
    if (existingLock) {
      ...
      if (error) {
        ...
        if (error.message.includes("Could not find the table") || 
            error.message.includes("does not exist")) {
          console.warn("meal_plan_locks table not found. Lock not created. Please run migration: npm run db:push");
          return;
        }
    ...
    // Create new lock
    const { error: insertError } = await supabase.from("meal_plan_locks").insert({
      user_id: args.userId,
  ...
  async recordChange(args: {
    ...
    if (error) {
      ...
      if (error.message.includes("Could not find the table") || 
          error.message.includes("does not exist")) {
        console.warn("meal_plan_changes table not found. Change not recorded. Please run migration: npm run db:push");
        return;
      }
  ...
  async getLocksForPlan(
    ...
    if (error) {
      ...
      throw new AppError(
        "DB_ERROR",
        `Failed to get locks: ${error.message}`
      );
    }
  ...
  async getChangesForPlan(
    ...
    if (error) {
      ...
      if (error.message.includes("Could not find the table") || 
          error.message.includes("does not exist")) {
        console.warn("meal_plan_changes table not found. Please run migration: npm run db:push");
        return [];
      }
