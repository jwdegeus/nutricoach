 ...
 */
function calculateAvailableG(
  pantry: PantryAvailability | undefined
): number {
  if (!pantry) {
    return 0;
  }
...
export class MealPlannerShoppingService {
  ...
  async buildCoverage(args: {
    ...
    const coveragePct =
      totalRequiredG === 0
        ? 100
        : Math.round(((totalRequiredG - totalMissingG) / totalRequiredG) * 100 * 10) / 10;
