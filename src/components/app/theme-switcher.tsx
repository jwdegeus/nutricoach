...
export function ThemeSwitcher({ variant = "navbar", className, ...props }: ThemeSwitcherProps) {
  ...
  const ButtonComponent = variant === "navbar" ? NavbarItem : Button;
  ...
  return (
    <Dropdown>
      <DropdownButton as={ButtonComponent} aria-label="Thema wijzigen" className={className} {...props}>
        {getIcon()}
      </DropdownButton>
      {mounted && (
